...
class Dataset(data.Dataset):
    ...
    def process_info(self, img_id):
        ...
        path = os.path.join(self.data_root, self.coco.loadImgs([img_id])[0]['file_name'])
        return anno, path, img_id
    
    def rle_to_mask(self, ann):
    ...
    def read_original_data(self, anno, path):
        img = cv2.imread(path)
        instance_polys = [[np.array(self.rle_to_mask(obj)).reshape(-1, 2)[:,::-1]] for obj in anno
                            ]
    ...
    def __getitem__(self, index):
        ...
        # for i in range(len(instance_polys)):
            # cv2.drawContours(img, instance_polys[i][0].reshape(-1,1,2).astype(int), -1, (0, 255, 0), 2)
        # cv2.imwrite("save_aed/" + str(random.randint(1,10000)) + ".jpg", img)  
